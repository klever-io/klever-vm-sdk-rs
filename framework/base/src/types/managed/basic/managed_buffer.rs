use crate::{
    ...
    types::{heap::BoxedBytes, ManagedBufferBuilder, ManagedType, StaticBufferRef},
};
...
impl<M: ManagedTypeApi> ManagedBuffer<M> {
    ...
    pub fn hex_expr(&self) -> ManagedBuffer<M> {
        let mut result = ManagedBufferBuilder::new_from_slice(b"0x");
...
impl<M: ManagedTypeApi> TypeAbi for ManagedBuffer<M> {
    type Unmanaged = klever_sc_codec::Vec<u8>;
