...
pub fn extract_wasm_imports(output_wasm_path: &str) -> Vec<String> {
    let wasm_data = fs::read(output_wasm_path)
        .expect("error occurred while extracting imports from .wasm: file not found");

    parse_wasm_imports(wasm_data).expect("error occurred while extracting imports from .wasm ")
}
