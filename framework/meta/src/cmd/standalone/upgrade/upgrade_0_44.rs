...
/// Migrate `0.43.3` to `0.44.0`, including the version bump.
pub fn upgrade_to_44_0(dir: &RelevantDirectory) {
    if dir.dir_type == DirectoryType::Contract {
        v_0_44_prepare_meta(&dir.path);
    }
...
fn v_0_44_prepare_meta(sc_crate_path: &Path) {
    ...
    if let Some(meta_dep) = deps.get_mut("klever-sc-meta") {
        let meta_dep_table = meta_dep
            .as_table_mut()
            .expect("klever-sc-meta dependency expected to be given as a table");
