...
use crate::version_history::is_template_with_autogenerated_wasm;
use crate::{
...
impl TemplateAdjuster {
    ...
    fn rename_in_tests(&self) {
        ...
        let old_kleversc = kleversc_file_name(&self.metadata.name);
        let new_kleversc = kleversc_file_name(&self.target.new_name);

        self.rename_in_scenarios(&old_wasm, &new_wasm);
        self.rename_in_scenarios(&old_kleversc, &new_kleversc);

        queries.push(Query::substring(&old_wasm, &new_wasm));
        queries.push(Query::substring(&old_kleversc, &new_kleversc));
    ...
    fn rename_in_interactor(&self) {
        ...
        let queries = vec![Query::substring(&old_kleversc, &new_kleversc)];
    ...
    fn rename_solution_files(&self) {
        ...
        if self.metadata.src_file == "lib.rs" {
            ...
            if src_lib_path.exists() {
                std::fs::rename(&src_lib_path, &src_new_path)
                    .expect("failed to rename src/lib.rs");
            }
...
fn kleversc_file_name(name: &str) -> String {
    format!("{name}.kleversc.json",)
}
