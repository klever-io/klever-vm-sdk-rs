...
pub fn generate_proxy_endpoint(m: &Method, endpoint_name: String) -> proc_macro2::TokenStream {
    ...
    if token_count > 0 || nonce_count > 0 || payment_count > 0 {
        ...
        if token_count == 0 && nonce_count == 0 {
            payment_type = quote! { klever_sc::types::KlvPayment<Self::Api> };
            payment_init = quote! { .klv(#payment_expr) };
        } else {
            payment_type = quote! { klever_sc::types::KdaTokenPayment<Self::Api> };
            payment_init = quote! { .payment(
                klever_sc::types::KdaTokenPayment::new(
                    #token_expr,
                    #nonce_expr,
                    #payment_expr,
                )
        ...
        payment_type = quote! { MultiKdaPayment<Self::Api> };
        payment_init = quote! { .multi_kda(#multi_expr.clone_value()) };
    } else {
        payment_type = quote! { () };
        payment_init = quote! {};
    }
    ...
    let return_type = quote! {
        klever_sc::types::Tx<
            klever_sc::types::TxScEnv<Self::Api>,
            (),
            Self::To,
            #payment_type,
            (),
            klever_sc::types::FunctionCall<Self::Api>,
            klever_sc::types::OriginalResultMarker<#original_type>,
        >
    };
    ...
    let sig = quote! {
        ...
        #msig {
            klever_sc::types::TxBaseWithEnv::new_tx_from_sc()
...
pub fn generate_proxy_deploy(init_method: &Method) -> proc_macro2::TokenStream {
    ...
    let return_type = quote! {
        klever_sc::types::Tx<
            ...
            #payment_type,
            (),
            klever_sc::types::DeployCall<klever_sc::types::TxScEnv<Self::Api>, ()>,
            klever_sc::types::OriginalResultMarker<#original_type>,
        >
    };
