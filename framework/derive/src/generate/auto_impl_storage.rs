...
fn generate_key_snippet(key_args: &[MethodArgument], identifier: &str) -> proc_macro2::TokenStream {
    ...
    let key_appends: Vec<proc_macro2::TokenStream> = key_args
        ...
        .collect();
    quote! {
        let mut ___key___ = klever_sc::storage::StorageKey::<Self::Api>::new(
            &#id_literal[..],
        );
        #(#key_appends)*
    }
    ...
    match m.return_type.clone() {
        ...
        syn::ReturnType::Type(_, _ty) => {
            quote! {
                #msig {
                    #key_snippet
                    klever_sc::storage::storage_get(
                        klever_sc::types::ManagedRef::new(&___key___),
                    )
                }
            }
        },
    ...
    match m.return_type.clone() {
        ...
        syn::ReturnType::Type(_, ty) => {
            quote! {
                #msig {
                    #key_snippet
                    <#ty as klever_sc::storage::mappers::StorageMapper<Self::Api>>::new(
                        ___key___
                    )
                }
            }
...
pub fn generate_is_empty_impl(m: &Method, identifier: &str) -> proc_macro2::TokenStream {
    ...
    quote! {
        #msig {
            #key_snippet
            klever_sc::storage::storage_get_len(
                klever_sc::types::ManagedRef::new(&___key___),
            ) == 0
        }
    }
...
pub fn generate_clear_impl(m: &Method, identifier: &str) -> proc_macro2::TokenStream {
    ...
    quote! {
        #msig {
            #key_snippet
            klever_sc::storage::storage_clear(
                klever_sc::types::ManagedRef::new(&___key___),
            );
        }
    }
