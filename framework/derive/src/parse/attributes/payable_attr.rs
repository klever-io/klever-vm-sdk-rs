...
fn extract_token_identifier(attr: &syn::Attribute) -> Option<String> {
    match attr.meta.clone() {
        syn::Meta::Path(_) => {
            panic!("attribute needs 1 string argument: Replace with #[payable(\"*\")] or #[payable(\"KLV\")]")
        },
        syn::Meta::List(list) => {
            ...
            let ticker = match iter.next() {
                Some(proc_macro2::TokenTree::Literal(literal)) => {
                    ...
                    assert!(
                        !clean.is_empty(),
                        "ticker can not be empty. attribute needs 1 string argument: Replace with #[payable(\"*\")] or #[payable(\"KLV\")"
                    );
                    clean
                },
                Some(_) => panic!("expected a string as argument"),
                None => panic!("argument can not be empty. attribute needs 1 string argument: Replace with #[payable(\"*\")] or #[payable(\"KLV\")"),
            };
            assert!(
                iter.next().is_none(),
        ...
        syn::Meta::NameValue(_) => panic!(
            "attribute can not be name value. attribute needs 1 string argument: \"*\" or \"KLV\""
        ),
    }
}
