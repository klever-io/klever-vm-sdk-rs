    ...
    match fields {
        syn::Fields::Named(fields_named) => fields_named
            ...
            .map(|field| {
                ...
                quote! {
                    <#type_name as klever_sc::types::ManagedVecItem>::SKIPS_RESERIALIZATION
                }
            })
            .collect(),
        _ => {
    ...
    match fields {
        syn::Fields::Named(fields_named) => fields_named
            ...
            .map(|field| {
                ...
                let type_name = &field.ty;
                quote! {
                    #field_ident: klever_sc::types::ManagedVecItem::from_byte_reader(|bytes| {
                        let next_index = index + <#type_name as klever_sc::types::ManagedVecItem>::payload_size();
    ...
    match fields {
        syn::Fields::Named(fields_named) => fields_named
            ...
            .map(|field| {
                ...
                let type_name = &field.ty;
                quote! {
                    klever_sc::types::ManagedVecItem::to_byte_writer(&self.#field_ident, |bytes| {
                        let next_index = index + <#type_name as klever_sc::types::ManagedVecItem>::payload_size();
...
fn generate_payload_buffer_snippet() -> proc_macro2::TokenStream {
    quote! {
        let mut payload = <Self::PAYLOAD as klever_sc::types::ManagedVecItemPayload>::new_buffer();
        let payload_slice = klever_sc::types::ManagedVecItemPayload::payload_slice_mut(&mut payload);
    }
}
...
fn enum_derive(data_enum: &syn::DataEnum, ast: &syn::DeriveInput) -> TokenStream {
    ...
    let gen = quote! {
        impl #impl_generics klever_sc::types::ManagedVecItem for #name #ty_generics #where_clause {
            type PAYLOAD = klever_sc::types::ManagedVecItemPayloadBuffer<1>;
...
fn struct_derive(data_struct: &syn::DataStruct, ast: &syn::DeriveInput) -> TokenStream {
    ...
    let gen = quote! {
        impl #impl_generics klever_sc::types::ManagedVecItem for #name #ty_generics #where_clause {
            type PAYLOAD = <#payload_nested_tuple as klever_sc::types::ManagedVecItemNestedTuple>::PAYLOAD;
