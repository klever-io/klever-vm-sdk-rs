...
fn type_payload_size(type_name: &syn::Type) -> proc_macro2::TokenStream {
    quote! {
        <#type_name as klever_sc::types::ManagedVecItem>::PAYLOAD_SIZE
    }
}
...
fn generate_array_init_snippet(ast: &syn::DeriveInput) -> proc_macro2::TokenStream {
    ...
    let self_expr = if ast.generics.params.is_empty() {
        ...
        quote! { #name <klever_sc::api::uncallable::UncallableApi> }
    };
    quote! {
        const SELF_PAYLOAD_SIZE: usize = <#self_expr as klever_sc::types::ManagedVecItem>::PAYLOAD_SIZE;
