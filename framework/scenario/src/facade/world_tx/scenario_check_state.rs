...
impl<'w> CheckStateBuilder<'w> {
        ...
        V: AnnotatedValue<ScenarioTxEnvData, BigUint<StaticApi>>,
    {
        ...
        match &mut self.current_account.kda {
            CheckKdaMap::Unspecified | CheckKdaMap::Star => {
                let mut new_kda_map = BTreeMap::new();
                let _ = new_kda_map.insert(token_id_key, CheckKda::Short(balance_value));
                ...
                self.current_account.kda = CheckKdaMap::Equal(new_check_kda_map);
            },
            CheckKdaMap::Equal(check_kda_map) => {
                if check_kda_map.contents.contains_key(&token_id_key) {
                    let prev_entry = check_kda_map.contents.get_mut(&token_id_key).unwrap();
                    match prev_entry {
                        CheckKda::Short(prev_balance_check) => *prev_balance_check = balance_value,
        ...
        T: AnnotatedValue<ScenarioTxEnvData, ManagedBuffer<StaticApi>>,
    {
        ...
        if let CheckKdaMap::Unspecified = &self.current_account.kda {
            ...
            let mut new_kda_map = BTreeMap::new();
            let _ = new_kda_map.insert(token_id_key, check_kda);
