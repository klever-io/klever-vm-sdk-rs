use klever_sc::{
    ...
    types::{
        BigUint, KdaTokenPayment, ManagedByteArray, ManagedType, ManagedVecItemPayload,
        TokenIdentifier,
    },
};
...
// cargo expand --test derive_managed_vec_item_kda_token_payment_test > expanded.rs
...
pub struct ManagedStructWithToken<M: ManagedTypeApi> {
    pub token: klever_sc::types::KdaTokenPayment<M>,
    pub num: u32,
...
fn struct_with_numbers_static() {
    assert_eq!(
        <ManagedStructWithToken<StaticApi> as klever_sc::types::ManagedVecItem>::payload_size(),
        28
    );
    assert!(
        !<ManagedStructWithToken<StaticApi> as klever_sc::types::ManagedVecItem>::SKIPS_RESERIALIZATION
    );
}
...
fn struct_to_bytes_writer() {
    ...
    let mut payload = <ManagedStructWithToken<StaticApi> as klever_sc::types::ManagedVecItem>::PAYLOAD::new_buffer();
    ...
    <ManagedStructWithToken<StaticApi> as klever_sc::types::ManagedVecItem>::to_byte_writer(
        &s,
        |bytes| {
...
fn struct_from_bytes_reader() {
    let s = ManagedStructWithToken::<StaticApi> {
        token: KdaTokenPayment::new(TokenIdentifier::from("MYTOKEN-12345"), 0u64, 42u64.into()),
        num: 0x12345,
        eth_address_1: ManagedByteArray::new_from_bytes(&[1u8; 20]),
        eth_address_2: ManagedByteArray::new_from_bytes(&[2u8; 20]),
    };
    ...
    let struct_from_bytes =
        <ManagedStructWithToken<StaticApi> as klever_sc::types::ManagedVecItem>::from_byte_reader(
            |bytes| {
