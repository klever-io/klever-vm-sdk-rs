...
fn world() -> ScenarioWorld {
    ...
    blockchain.register_contract(CF_PATH_EXPR, crowdfunding_kda::ContractBuilder);
    blockchain
}
...
fn crowdfunding_scenario_rust_test() {
    ...
    let cf_token_id_value = "CROWD-1234"; // when passing as argument
    let cf_token_id = "str:CROWD-1234"; // when specifying the token transfer
    let mut cf_sc = ContractInfo::<crowdfunding_kda::Proxy<StaticApi>>::new("sc:crowdfunding");

    // setup owner and crowdfunding SC
    world.start_trace().set_state_step(
        SetStateStep::new()
            .put_account(owner_addr, Account::new())
            .new_address(owner_addr, 1, &cf_sc),
    );
    ...
    world.sc_deploy(
        ScDeployStep::new()
            ...
            .call(cf_sc.init(
                2_000u32,
                deadline,
                TokenIdentifier::from(cf_token_id_value),
            )),
    );
    ...
    world
        .set_state_step(SetStateStep::new().put_account(
            first_user_addr,
            Account::new().kda_balance(cf_token_id, 1_000u64),
        ))
        .set_state_step(SetStateStep::new().put_account(
            second_user_addr,
            Account::new().kda_balance(cf_token_id, 1_000u64),
    ...
    world
        .sc_call(
            ScCallStep::new()
                ...
                .to(&cf_sc)
                .kda_transfer(cf_token_id, 0u64, 1_000u64)
                .call(cf_sc.fund()),
        )
        .check_state_step(
            CheckStateStep::new()
                .put_account(
                    first_user_addr,
                    CheckAccount::new().kda_balance(cf_token_id, 0u64),
                )
                .put_account(
                    &cf_sc,
                    CheckAccount::new().kda_balance(cf_token_id, 1_000u64),
                ),
        );
    ...
    world
        .sc_call(
            ScCallStep::new()
                ...
                .to(&cf_sc)
                .kda_transfer(cf_token_id, 0u64, 500u64)
                .call(cf_sc.fund()),
        )
        .check_state_step(
            CheckStateStep::new()
                .put_account(
                    second_user_addr,
                    CheckAccount::new().kda_balance(cf_token_id, 500u64),
                )
                .put_account(
                    &cf_sc,
                    CheckAccount::new().kda_balance(cf_token_id, 1_500u64),
                ),
        );
    ...
    world
        ...
        .check_state_step(
            CheckStateStep::new()
                ...
                .put_account(&cf_sc, CheckAccount::new().kda_balance(cf_token_id, 0u64)),
        );
    ...
    world
        ...
        .check_state_step(
            CheckStateStep::new()
                .put_account(
                    owner_addr,
                    CheckAccount::new().kda_balance(cf_token_id, 2_000u64),
                )
                .put_account(cf_sc, CheckAccount::new().kda_balance(cf_token_id, 0u64)),
        );
