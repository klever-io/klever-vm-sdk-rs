...
pub trait Lottery {
    ...
    fn start_lottery(
        ...
        match opt_burn_percentage {
            OptionalValue::Some(burn_percentage) => {
                require!(!token_identifier.is_klv(), "KLV can't be burned!");

                let properties = self
                    .blockchain()
                    .get_kda_properties(&token_identifier.clone());
                require!(
                    properties.can_burn,
    ...
    fn distribute_prizes(&self, lottery_name: &ManagedBuffer) {
        ...
        if burn_percentage > 0 {
            ...
            let kda_token_id = info.token_identifier.clone();
            let properties = self.blockchain().get_kda_properties(&kda_token_id);
            if properties.can_burn {
                self.send().kda_burn(&kda_token_id, 0, &burn_amount);
            }
