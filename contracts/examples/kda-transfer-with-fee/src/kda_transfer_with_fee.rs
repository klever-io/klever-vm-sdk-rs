...
pub trait KdaTransferWithFee {
    ...
    fn claim_fees(&self) {
        ...
        for ((token, nonce), amount) in self.paid_fees().iter() {
            fees.push(KdaTokenPayment::new(token, nonce, amount))
        }
    ...
    fn transfer(&self, address: ManagedAddress) {
        require!(
            *self.call_value().klv_value() == 0,
            "KLV transfers not allowed"
        );
    ...
    fn get_payment_after_fees(
        &self,
        fee: Fee<Self::Api>,
        payment: &KdaTokenPayment<Self::Api>,
    ...
    fn calculate_fee(
        &self,
        fee: &Fee<Self::Api>,
        mut provided: KdaTokenPayment<Self::Api>,
    ) -> KdaTokenPayment<Self::Api> {
        match fee {
