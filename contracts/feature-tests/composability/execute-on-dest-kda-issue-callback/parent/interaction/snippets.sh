ALICE="/home/elrond/klever-vm-sdk/testwallets/latest/users/alice.pem"
...
KDA_ISSUE_COST=5000000000000000000

TOKEN_DISPLAY_NAME=0x5772617070656445676c64  # "WrappedKlv"
TOKEN_TICKER=0x5745474c44  # "WKLV"
...
executeOnDestIssueToken() {
    mxpy --verbose contract call ${ADDRESS} --recall-nonce --pem=${ALICE} --gas-limit=200000000 --value=${KDA_ISSUE_COST} --function="executeOnDestIssueToken" --arguments ${TOKEN_DISPLAY_NAME} ${TOKEN_TICKER} ${INITIAL_SUPPLY} --send --proxy=${PROXY} --chain=${CHAIN_ID}
}
...
getWrappedKlvTokenIdentifier() {
    getChildContractAddress
    mxpy --verbose contract call ${CHILD_ADDRESS} --recall-nonce --pem=${ALICE} --gas-limit=50000000 --function="getWrappedKlvTokenIdentifier" --send --proxy=${PROXY} --chain=${CHAIN_ID}
}
