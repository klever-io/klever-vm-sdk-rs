...
getFungibleKdaBalance() {
    mxpy --verbose contract call ${ADDRESS} --recall-nonce --pem=${ALICE} --gas-limit=50000000 --function="getFungibleKdaBalance" --arguments ${TOKEN_IDENTIFIER} --send --proxy=${PROXY} --chain=${CHAIN_ID}
    #mxpy --verbose contract query ${ADDRESS} --function="getFungibleKdaBalance" --arguments ${TOKEN_IDENTIFIER} --proxy=${PROXY}
}
