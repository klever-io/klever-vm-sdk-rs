...
class KdaTokenPayment(ManagedVecItem, ManagedType):
...
KLEVER_WASM_TYPE_HANDLERS = [
    ...
    (KDA_TOKEN_PAYMENT_TYPE, KdaTokenPayment),
...
def get_handler(type_name: str) -> Handler:
    for rust_type, handler_class in KLEVER_WASM_TYPE_HANDLERS:
...
def __lldb_init_module(debugger: SBDebugger, dict):
    ...
    for rust_type, handler_class in KLEVER_WASM_TYPE_HANDLERS:
        ...
        summary_command = f'type summary add -x "^{rust_type}$" -F {python_module_name}.{summary_function_name} --category klever-sc'
        debugger.HandleCommand(summary_command)
        # print(f"Registered: {summary_command}")

    # Enable categories
    debugger.HandleCommand('type category enable klever-sc')
