use crate::{
    tx_execution::{builtin_function_names::KDA_NFT_UPDATE_ATTRIBUTES_FUNC_NAME, BlockchainVMRef},
...
pub struct KDANftUpdateAttributes;

impl BuiltinFunction for KDANftUpdateAttributes {
    fn name(&self) -> &str {
        KDA_NFT_UPDATE_ATTRIBUTES_FUNC_NAME
    }
        ...
        F: FnOnce(),
    {
        if tx_input.args.len() != 3 {
            let err_result = TxResult::from_vm_error("KDANFTUpdateAttributes expects 3 arguments");
        ...
        let kda_nft_create_log = TxLog {
            ...
            endpoint: KDA_NFT_UPDATE_ATTRIBUTES_FUNC_NAME.into(),
            topics: vec![
